...
# Порядок и заголовки групп. Наружу не отдаются: фронтенд раскладывает метрики
# по группам сам, а подписи берёт из своего словаря (group.* в static/i18n.js) —
# как и подписи из CODE_META выше, которые он переводит по коду DP.
