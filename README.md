...
- **Пиксельный интерфейс, два набора тем** (у каждого светлый и тёмный вариант):
  ...
  развёрткой. Тема по умолчанию задаётся в админ-панели, переключатель в шапке меняет
...
- **Выпадающий список в шапке** любой страницы меняет оформление **только в этом
  браузере**: выбор кладётся в `localStorage` (ключ `bm-theme`) и важнее темы по
  умолчанию. Вернуть общую тему — кнопкой «Вернуть тему по умолчанию» в той же
  карточке админ-панели.
