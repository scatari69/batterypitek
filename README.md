...
- **Список устройств** (главная): строка на батарею — имя, шкала и уровень заряда (SOC),
  режим, **оценка времени до целевого уровня заряда** (по умолчанию 30 %), напряжение и ток.
...
- **Админ-панель `/admin`**: ввод реквизитов Tuya, проверка подключения,
  ...
  оформление и язык, настройка уведомлений, пароль доступа.
- **Пиксельный интерфейс, два набора тем** (у каждого светлый и тёмный вариант):
  Dracula — по умолчанию (палитра Dracula и её официальный светлый вариант Alucard,
  гарнитура Meslo LG), «Пиксель» — приборный VFD-дисплей и монохромный LCD с построчной
  развёрткой. Тема по умолчанию задаётся в админ-панели, переключатель в футере меняет
  оформление только в текущем браузере — см. [Оформление](#оформление).
- **Три языка**: русский, украинский, английский. Язык по умолчанию задаётся в
  админ-панели (на нём же приходят уведомления в Telegram), переключатель в футере
  меняет язык только в текущем браузере — см. [Язык](#язык).
- Крупные цифры заряда рисуются растровым шрифтом 5×7 прямо в SVG, поэтому для них
  внешние гарнитуры не нужны и всё работает офлайн.
- Автообновление, адаптив под телефон, запуск в Docker.

| | |
|---|---|
| ![Список](screenshots/devices-list-dracula-dark.png) | ![Админ](screenshots/admin-panel-dracula-dark.png) |
...
**Caddy запущен на хосте** (обычный systemd-сервис) — этот вариант работает с
настройками «из коробки»:
...
> кто может открыть соединение». Это безопасно ровно до тех пор, пока до приложения
> нельзя достучаться в обход прокси. Если вернёте публикацию порта на все интерфейсы
> (`"8000:8000"`), любой в сети сможет подделать эти заголовки.
...
|---|---|
| **Заряд ниже порога** | SOC ≤ «целевой уровень заряда, %» из «Общих параметров» |
...
**повтор напоминания** (0 — не повторять). Сам порог заряда задаётся один раз на
всё приложение — в «Общих параметрах», см. [«Время до N %»](#как-считается-время-до-n-).
...
## Как считается «время до N %»

Кулоновский метод: `(остаточная_Ah − N/100 × полная_Ah) / ток_разряда`.
...
**N — целевой уровень заряда**, общий для всего приложения: админ-панель →
«Общие параметры» → «Целевой уровень заряда, %» (ключ `target_soc` в
`settings.json`, bootstrap — переменная `TARGET_SOC`, по умолчанию 30). Он же
служит порогом уведомления о низком заряде в Telegram, поэтому отдельной
настройки порога в карточке уведомлений нет.
...
Заголовок `X-BM-Lang: ru｜uk｜en` в любом запросе задаёт язык ответа (сообщения
об ошибках, имена демо-устройств). Без него используется язык по умолчанию.
...
Наборов тем два, у каждого светлый и тёмный вариант плюс «авто» — по настройке
системы:

| Набор | Тёмная | Светлая | Особенности |
|---|---|---|---|
| **Dracula** (по умолчанию) | палитра [Dracula](https://draculatheme.com/) | палитра Alucard | без построчной развёртки, гарнитура Meslo LG |
| **Пиксель** | приборный VFD-дисплей | монохромный LCD | построчная развёртка дисплея, системная моноширинная гарнитура |

| | |
|---|---|
| ![Dracula, тёмная](screenshots/devices-list-dracula-dark.png) | ![Dracula, светлая](screenshots/devices-list-dracula-light.png) |
| ![Пиксель, тёмная](screenshots/devices-list-dark.png) | ![Пиксель, светлая](screenshots/dashboard-light.png) |
...
- **Админ-панель → «Оформление»** задаёт **тему по умолчанию**. Она хранится на
  сервере (`theme` в `data/settings.json`, bootstrap — переменная `THEME`) и
  действует на всех устройствах, где оформление не меняли вручную.
- **Выпадающий список в футере** дашборда и страницы устройства (в админ-панели —
  в шапке) меняет оформление **только в этом браузере**: выбор кладётся в
  `localStorage` (ключ `bm-theme`) и важнее темы по умолчанию. Вернуть общую тему —
  кнопкой «Вернуть тему по умолчанию» в той же карточке админ-панели.
...
Интерфейс переведён на **русский**, **украинский** и **английский**. Выбор
устроен так же, как у оформления, и в двух местах:

- **Админ-панель → «Язык»** задаёт **язык по умолчанию**. Он хранится на сервере
  (`language` в `data/settings.json`, bootstrap — переменная `LANGUAGE`) и
  действует на всех устройствах, где язык не меняли вручную. На нём же уходят
  **уведомления в Telegram** — у них браузера-получателя нет.
- **Выпадающий список в футере** дашборда и страницы устройства меняет язык
  **только в этом браузере**: выбор кладётся в `localStorage` (ключ `bm-lang`)
  и важнее общей настройки. Вернуть общий язык — кнопкой «Вернуть язык
  по умолчанию» в той же карточке админ-панели.

Переключение мгновенное, без перезагрузки страницы: словари всех трёх языков
лежат в `app/static/i18n.js` и грузятся вместе со страницей. Выбранный язык
уходит на сервер заголовком `X-BM-Lang`, поэтому сообщения об ошибках API и
имена демо-устройств тоже приходят на нём (серверные строки — `app/i18n.py`).

Подписи точек данных переводятся на клиенте по коду DP (`dp` в `i18n.js`);
для незнакомого кода показывается название его роли, а если и роль неизвестна —
сам код, как и раньше.

Чтобы добавить язык, нужно описать блок в `DICT` (`app/static/i18n.js`), такой же
набор ключей в `MESSAGES` (`app/i18n.py`) и добавить код в `LANGS` там же.
...
│   ├── metrics.py         # нормализация DP + оценка времени до целевого уровня
│   ├── notifier.py        # Telegram: фоновая проверка порогов + отправка
│   ├── i18n.py            # серверные строки: API, уведомления, демо-устройства
│   ├── demo.py            # демо-данные
...
│                          # i18n.js  — словари интерфейса (ru / uk / en)
...
Ради этого в футере дашборда и страницы устройства есть ссылка **«исходный
код»**. Адрес задаётся переменной `SOURCE_URL` (по умолчанию — репозиторий
апстрима): **в форке впишите свой репозиторий**, иначе ссылка будет вести не на
тот код, который вы раздаёте. Пустое значение убирает ссылку.
