...
- **Альтернативный набор тем Dracula** (светлая и тёмная) — палитра Dracula и её
  официальный светлый вариант Alucard, гарнитура Meslo LG, без построчной развёртки
  дисплея. Переключатель — в шапке любой страницы, см. [Оформление](#оформление).
...
Выпадающий список в шапке любой страницы (дашборд, устройство, админ-панель)
переключает оформление. Наборов два, у каждого светлый и тёмный вариант плюс
«авто» — по настройке системы:

| Набор | Тёмная | Светлая | Особенности |
|---|---|---|---|
| **Пиксель** (по умолчанию) | приборный VFD-дисплей | монохромный LCD | построчная развёртка дисплея, системная моноширинная гарнитура |
| **Dracula** | палитра [Dracula](https://draculatheme.com/) | палитра Alucard | без построчной развёртки, гарнитура Meslo LG |

| | |
|---|---|
| ![Dracula, тёмная](screenshots/devices-list-dracula-dark.png) | ![Dracula, светлая](screenshots/devices-list-dracula-light.png) |

Выбор хранится в `localStorage` браузера (ключ `bm-theme`), на сервер ничего не
уходит — у каждого устройства своё оформление. Тема применяется до первой
отрисовки, поэтому мигания при загрузке нет.
...
`pixel-dark` · `pixel-light` · `dracula-dark` · `dracula-light`. Чтобы добавить свой
набор, достаточно описать ещё один блок переменных и пункт в списке `CHOICES`.
...
│                          # theme.js — выбор оформления (Пиксель / Dracula)
